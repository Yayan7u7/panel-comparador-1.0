                        ...
                        <ul class="list-group list-group-flush">
                            <?php foreach ($top_positive_influencers as $influencer):
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    ...
                                    <span class="badge bg-success rounded-pill">Positivas: <?= $total_positive_mentions ?></span>
                                </li>
                            <?php endforeach; ?>
                        </ul>
                    <?php else: ?>
                        <div class="alert alert-info text-center">
                            No se encontraron influencers positivos para el período y plataformas seleccionados.
                        </div>
                    <?php endif; ?>
                </div>
            </div>
                        ...
                        <ul class="list-group list-group-flush">
                            <?php foreach ($top_negative_influencers as $influencer):
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    ...
                                    <span class="badge bg-danger rounded-pill">Negativas: <?= $total_negative_mentions ?></span>
                                </li>
                            <?php endforeach; ?>
                        </ul>
                    <?php else: ?>
                        <div class="alert alert-info text-center">
                            No se encontraron influencers negativos para el período y plataformas seleccionados.
                        </div>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <div class="col-12 mb-4">
            <div class="card shadow">
                ...
                <div class="card-body">
                    ...
                    <div class="loader" id="compareLoader"></div>
                    <div id="comparisonResultContainer" class="mt-4" style="display: none;">
...
<script src="assets/lib/spdf.umd.min.js"></script>
...
<script>
    ...
    // --- Lógica de Exportar CSV/PDF (MODIFICADA PARA PDF CLIENTE-SIDE) ---
    ...
    if (exportPdfBtn) {
        exportPdfBtn.addEventListener('click', function(e) {
            ...
            // Mostrar un mensaje de carga, ya que la generación de PDF puede tardar
            alert('Generando PDF... Esto puede tomar un momento.');

            // Se captura el 'body' completo para incluir todo el contenido visible
            const element = document.body;

            html2canvas(element, {
                scale: 2, // Aumenta la resolución para una mejor calidad en el PDF
                useCORS: true, // Importante si tienes imágenes de otros dominios
                logging: true // Para depuración en consola del navegador
            }).then(canvas => {
                ...
                const pageHeight = 297; // Altura de una página A4 en mm (297mm)
                // Calcula la altura de la imagen en el PDF manteniendo su relación de aspecto
                const imgHeight = canvas.height * imgWidth / canvas.width;
                let heightLeft = imgHeight; // Altura restante del contenido a colocar en el PDF

                let position = 0; // Posición vertical actual en el PDF

                // Añade la primera imagen (página)
                pdf.addImage(imgData, 'PNG', 0, position, imgWidth, imgHeight);
                heightLeft -= pageHeight; // Reduce la altura restante por el tamaño de una página

                // Mientras quede contenido por agregar, añade nuevas páginas
                while (heightLeft >= 0) {
                    position = heightLeft - imgHeight; // Calcula la posición para la siguiente parte de la imagen
                    pdf.addPage(); // Añade una nueva página
                    pdf.addImage(imgData, 'PNG', 0, position, imgWidth, imgHeight); // Añade el resto de la imagen
                ...
                const periodName = urlParams.get('period') || 'Personalizado';
                const platformName = urlParams.get('platform') || 'Todas';
                const dateSuffix = new Date().toISOString().slice(0,10); // Formato YYYY-MM-DD
                const filename = `reporte_menciones_${periodName}_${platformName}_${dateSuffix}.pdf`;

                pdf.save(filename); // Descarga el archivo PDF
            }).catch(error => {
                console.error('Error al generar el PDF:', error);
                alert('Hubo un error al generar el PDF. Revisa la consola del navegador para más detalles.');
            });
        });
    }
</script>
...
<script>
    document.getElementById('exportPdfBtn').addEventListener('click', function(e) {
        ...
        const elementToCapture = document.getElementById('content-to-export');

        // Identificar imágenes problemáticas (Brandmentions u otras URLs externas)
        const images = elementToCapture.querySelectorAll('img');
        const originalSrcs = [];

        // Reemplazar temporalmente las imágenes problemáticas con un placeholder
        images.forEach((img, index) => {
            originalSrcs[index] = img.src;
            if (img.src.includes('brandmentions.com') || !img.src.startsWith('data:') && !img.src.includes(window.location.origin)) {
                img.src = 'https://via.placeholder.com/25'; // Placeholder genérico
            }
        });
        ...
        html2canvas(elementToCapture, {
            ...
            // Restaurar imágenes en caso de error
            images.forEach((img, index) => {
                if (originalSrcs[index]) {
                    img.src = originalSrcs[index];
                }
